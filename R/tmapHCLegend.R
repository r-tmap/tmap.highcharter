...
tmapHC_legend = function(cmp, deck, o, orientation) {

	# group = "tmp" # TODO
	# leg_className = paste("info legend", gsub(" ", "", group, fixed = TRUE))
	# layerId =  paste0("legend", sprintf("%02d", .TMAP_HC$leg_id)) # "legend401" #todo
	# .TMAP_HC$leg_id = .TMAP_HC$leg_id + 1
	# 
	...
	deck2 = if (cmp$type == "none") {
		...
		deck |> hc::add_legend(colors = gp2$fillColor, labels = cmp$labels, pos = legpos, title = cmp$title)
	}
	deck2
...
#' @export
tmapHCLegPlot.tm_legend_standard_portrait = function(comp, deck, o) {
	tmapHC_legend(comp, deck, o, orientation = "vertical")
}

#' @export
tmapHCLegPlot.tm_legend_standard_landscape = function(comp, deck, o) {
	tmapHC_legend(comp, deck, o, orientation = "horizontal")
}
...
#' @name tmapHCLegend
#' @rdname tmapHC
tmapHCComp = function(comp, o, facet_row = NULL, facet_col = NULL, facet_page, class, stack, stack_auto, pos.h, pos.v, bbox) {
